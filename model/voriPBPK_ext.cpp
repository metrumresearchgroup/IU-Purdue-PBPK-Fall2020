...
kd      = 0.849 * 10  //displacement from gut lumen rate constant
...
double Vre = WEIGHT - 
  (Vli + Vki + Vsp + Vhe + Vlu + Vbo + Vbr + Vmu + Vad + VguWall + Vbl); //volume of rest of the body compartment
...
double f = 1;
if(CgutLumen > S_lumen) f = 0;

//ODEs
dxdt_GUTLUMEN = -kd * VguLumen * (f * CgutLumen + (1 - f) * S_lumen) - 
  kt * GUTLUMEN;
dxdt_GUTWALL  = kd * VguLumen * (f * CgutLumen + (1 - f) * S_lumen) - 
  ka * GUTWALL - 
  CLintGut * CgutWall;
dxdt_GUT      = ka * GUTWALL + 
